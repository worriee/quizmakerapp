...
6. **User Interaction (chat)**: User types and sends a message in `ChatInterface`. `handleSendMessage()` in `App.jsx` appends user message to `history`, calls `POST /api/chat` with the conversation so far. The backend (`api/index.js`) verifies the JWT via `authenticate` middleware, then calls `handleChat()` from `api/ai.js`.
